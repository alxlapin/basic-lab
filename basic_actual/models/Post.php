...
class Post extends ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['post_author_id', 'post_project_id', 'post_type', 'post_views'], 'integer'],
            [['post_announce', 'post_desc'], 'string'],
            [['post_title'], 'string', 'max' => 255],
            [['post_author_id'], 'exist', 'skipOnError' => true, 'targetClass' => User::className(), 'targetAttribute' => ['post_author_id' => 'id']],
            [['post_project_id'], 'exist', 'skipOnError' => true, 'targetClass' => Project::className(), 'targetAttribute' => ['post_project_id' => 'id']],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            'post_project_id' => 'Новость по проекту'
        ];
    }

    public static function getTopNews() {
        return self::find()->with('author', 'tags', 'storages')
            ->where(['post_type' => self::STATUS_TOP])
    ...
    public static function getLatestNews() {
        return self::find()->with('author', 'tags', 'storages')
            ->where(['post_type' => self::STATUS_COMMON])
            ->orderBy('post_date DESC')
            ->limit(5)
            ->all();
    }
    ...
    {
        return $this->hasOne(Project::className(), ['id' => 'post_project_id']);
    }
